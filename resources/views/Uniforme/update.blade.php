...
<head>
    ...
    <title>Détails Des Uniformes</title>
...
<div class="container mt-4">
    <div class="card">
        <div class="card-header bg-primary text-white">
            <h3 class="mb-0">
                Détails des Uniformes pour {{$materiel->nom_eleve}}
            </h3>
        </div>
        <div class="card-body">
           
                <form action="{{ route('uniforme.update', ['id' => $materiel->id]) }}" method="POST">
                    @csrf
                    <div class="form-group">
                        <label for="materiel">État des Uniformes :</label>
                        <select name="uniforme" id="materiel" class="form-control">
                            <option value="ok" {{ $materiel->uniforme === 'ok' ? 'selected' : '' }}>OK</option>
                            <option value="non_ok" {{ $materiel->uniforme === 'non_ok' ? 'selected' : '' }}>NON OK</option>
                        </select>
                    </div>

                    <div class="form-group" id="materiel_restante_group" style="{{ $materiel->uniforme === 'non_ok' ? '' : 'display:none;' }}">
                        <label for="reste">Liste des Uniformes Manquant :</label>
                        <input type="text" name="reste" value="{{ $materiel->reste }}" class="form-control" placeholder="Uniforme manquant">
                    </div>
