...
<body class="flex h-screen bg-gray-100">
  ...
  <div class="flex-1 flex flex-col">
    ...
    <header class="bg-red-600 text-white shadow p-4 flex justify-between items-center">
      <h1 class="text-lg font-semibold"> M & F (Liste des candidats)</h1>
   ...
   <main class="flex-1 p-6">
    ...
    <h2 class="text-2xl font-bold">Liste Des Candidats Par ordre alphabetique pour la Filière  M & F</h2>
    <div class="flex space-x-4">
        <!-- Bouton Tri Alphabétique -->
        <a href="{{ route('admin_concours.finance', ['alphabetical' => true]) }}" 
...
<div id="roomModal" class="hidden fixed inset-0 bg-gray-600 bg-opacity-50 flex items-center justify-center p-4 z-50">
    <div class="bg-white rounded-lg shadow-xl w-full max-w-md">
        <div class="p-6">
            ...
            <form method="POST" action="{{ route('candidates.assign-rooms') }}">
                @csrf
...
<div class="flex justify-end space-x-4 mb-6">
    <a href="{{ route('candidates.print-rooms') }}" target="_blank"
    ...
    <form method="POST" action="{{ route('candidates.share-rooms.finance') }}">
        @csrf
