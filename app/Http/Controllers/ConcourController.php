...
use App\Models\Geoscience;
use App\Models\ManagementFinance;
...
class ConcourController extends Controller
{
   ...
   public function geoscience(){
    return view('Concours.geoscience');
   }
   public function finance(){
    $dateActuelle = now()->format('Y-m-d');
        
        $concoursOuvert = DB::table('sessions_concours')
            ->where('nom_filiere', 'M&F')
        ...
        if (!$concoursOuvert) {
            ...
            return redirect()->back()->with('error', 'Aucun concours M&F ouvert actuellement');
            
        }
    
    return view('Concours.finance');
   }
   ...
   public function store_finance(Request $request)
{
    ...
    try {
        ...
        $id = DB::table('management_finances')->insertGetId($insertData);
...
public function shareRoomsFinance(Request $request)
{
    ...
    $candidates = ManagementFinance::whereIn('id', $candidateIds)
                          ->get()
    ...
    $generalistes = DB::table('management_finances')->pluck('email')->toArray();
    $pdf = PDF::loadView('Concours.Admin.partagemail', compact('candidates'));
    
    foreach ($generalistes as $email) {
        Mail::to($email)->send(new FinanceMail($candidates, $pdf));
    }
...
public function shareWithGeosciences(Request $request)
{
    ...
    try {
        ...
        $emails = DB::table('management_finaces')->pluck('email')->toArray();
