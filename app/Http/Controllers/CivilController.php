...
use App\Mail\CivilMail;
use App\Mail\FinanceResultat;
...
class CivilController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('Concours.Civil.index');
    }
    
   public function store_civil(Request $request)
   {
       ...
       try {
           ...
           $id = DB::table('civils')->insertGetId($insertData);
           ...
           Log::error('Erreur inscription civil: '.$e->getMessage());
   ...
   public function civil(Request $request)
    {
        ...
        $query = DB::table('civils')->orderBy('created_at', 'asc');
        ...
        return view('Concours.Civil.dashboard', compact('candidates'));
    }
    public function showAttribution(Request $request)
    {
        // Tri par défaut (date décroissante)
        $query = DB::table('civils')->orderBy('created_at', 'desc');
        ...
        return view('Concours.Civil.salle', compact('candidates'));
    }
    ...
    public function shareRooms(Request $request)
    {
        ...
        $candidates = Civil::whereIn('id', $candidateIds)
                              ->get()
        ...
        $generalistes = DB::table('civils')->pluck('email')->toArray();
        $pdf = PDF::loadView('Concours.Civil.partagemail', compact('candidates'));
        
        foreach ($generalistes as $email) {
            Mail::to($email)->send(new CivilMail($candidates, $pdf));
        }
    ...
    public function shareWithGeosciences(Request $request)
{
    ...
    try {
        ...
        $emails = DB::table('civils')->pluck('email')->toArray();
        ...
        foreach ($emails as $email) {
            Mail::to($email)->send(new FinanceResultat(
                $request->subject,
                $request->message,
                $fullPath
            ));
