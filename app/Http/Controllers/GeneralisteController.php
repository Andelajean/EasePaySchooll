...
use App\Mail\GeneralisteMailPdf;
use App\Mail\GeneralisteResultat;
use App\Models\Generaliste;
...
class GeneralisteController extends Controller
{
    ...
    public function index(){
        ...
        return view('Concours.Inge.index');
    }
    
   public function store_inge(Request $request)
   {
       ...
       try {
           ...
           $id = DB::table('generalistes')->insertGetId($insertData);
           ...
           Log::error('Erreur inscription inge: '.$e->getMessage());
   ...
   public function inge(Request $request)
    {
        // Tri par défaut (date décroissante)
        $query = DB::table('generalistes')->orderBy('created_at', 'asc');
        ...
        return view('Concours.Inge.dashboard', compact('candidates'));
    }
    public function showAttribution(Request $request)
    {
        // Tri par défaut (date décroissante)
        $query = DB::table('generalistes')->orderBy('created_at', 'desc');
        ...
        return view('Concours.Inge.salle', compact('candidates'));
    }
    ...
    public function shareRooms(Request $request)
{
    ...
    $candidates = Generaliste::whereIn('id', $candidateIds)
                          ->get()
    ...
    $generalistes = DB::table('generalistes')->pluck('email')->toArray();
    $pdf = PDF::loadView('Concours.Inge.partagemail', compact('candidates'));
    
    foreach ($generalistes as $email) {
        Mail::to($email)->send(new GeneralisteMailPdf($candidates, $pdf));
    }
...
public function shareWithGeosciences(Request $request)
{
    ...
    try {
        ...
        $emails = DB::table('generalistes')->pluck('email')->toArray();
        ...
        foreach ($emails as $email) {
            Mail::to($email)->send(new GeneralisteResultat(
                $request->subject,
                $request->message,
                $fullPath
            ));
...
private function callMomoApi(Request $request, $id_paiement, $montantMajore)
    {
        try {
            ...
            $data = [
                ...
                'return_url' => route('recu_generaliste', ['id_paiement' => $id_paiement]),
                'notify_url' => route('recu_generaliste', ['id_paiement' => $id_paiement]),
                'user' => $request->nom,
    ...
    public function generaliste_finance(Request $request)
{
    try {
        ...
        try {
          ...
          //  dd(session('file_paths')); // Vérifie si la photo est bien sauvegardée
           ...
           // Gestion de la récupération et stockage de la photo
            ...
            $candidat = Generaliste::create($insertData);
    
           // Création du paiement en base de données
        $paiement = PaiementSaintJean::create([
            'filiere_concours' => 'Ingenieur Generaliste (INGE)',
